...
// storeSeq numbers the fixture databases so that every call gets its own file,
// even twice inside one test and even when tests run in parallel.
...
func newTestStore(t *testing.T) *Store {
	t.Helper()
	path := filepath.Join(integrationDir, fmt.Sprintf("store-%d.db", storeSeq.Add(1)))
