...
conformance harnesses — small enough to read in five minutes, structured,
hardened and measured the way a real project is.
...
go test -bench='BenchmarkList' -run='^$' -count=10 ./internal/store
go vet ./...
govulncheck ./...
```
...
internal/store   storage: concurrency-safe in-memory Store and its tag index
internal/remote  the one outbound call: bounded client, redacted errors
docs/adr         the decisions that were expensive to change
docs/perf        committed profiles and benchmark runs behind PERF.md
SECURITY.md      trust boundaries, inputs, secrets, dependencies, accepted risk
PERF.md          the one optimization: what was slow, how it was measured, what it cost
```
