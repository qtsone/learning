...
// DefaultRules is this service's access-control model, and it is deliberately
// small enough to read in one sitting: three roles, six actions, one
// relationship rule. Handlers name actions; only this table names roles, so
// "who may update a task" changes here and nowhere else.
//
// Task deletion is absent on purpose. The route exists, so the only thing
// standing between a caller and a DELETE is the missing rule — which is what
// deny-by-default buys you.
var DefaultRules = map[Action]Rule{
	ActionTaskList: {
		Roles: Set(RoleMember, RoleAuditor, RoleAdmin),
	},
	ActionTaskRead: {
		Roles:        Set(RoleMember, RoleAuditor, RoleAdmin),
		RequireOwner: true,
		OwnerExempt:  Set(RoleAuditor, RoleAdmin),
	},
	ActionTaskCreate: {
...
//
// So the question goes to the policy: may this subject read a task somebody
// else owns? If yes, ownership narrows nothing and they see everything; if no,
// they see their own rows and nothing else. Change the rule table and this
// follows, which is the whole point.
func (p *Policy) ScopeFor(sub Subject) Scope {
	someoneElse := Resource{ID: "*", OwnerID: "\x00other:" + sub.UserID}
	if p.Allows(sub, ActionTaskRead, someoneElse) {
		return Scope{All: true}
	}
	return Scope{OwnerID: sub.UserID}
}
